...
include("plotting.jl")
export load_inputs, save_variables, ssap_model
...
function load_inputs(product_path, shelf_path)
    ...
    return products, shelves, blocks, modules, P_b, S_m, G_p, H_s,
           L_p, P_ps, D_p, N_p_min, N_p_max, W_p, W_s, M_p, M_s_min, M_s_max, R_p, L_s, H_p, SK_p
end

"""Save the values of all variables into JSON file."""
function save_variables(model:: Model; filepath="solution.json")
    # TODO: handle data types
    variables = Dict(k => Array(value.(v)) for (k, v) in model.obj_dict)
