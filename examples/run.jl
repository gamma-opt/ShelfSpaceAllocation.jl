...
# # Fix the block width
# @constraint(model, [b = parameters.blocks, s = parameters.helves],
...
p1 = plot_planograms(
    products, shelves, blocks, S_m, P_b, H_s, H_p, W_p, W_s, SK_p, n_ps, o_s,
    x_bs)
for (i, p) in enumerate(p1)
    savefig(p, joinpath(output_dir, "planogram_$i.svg"))
end

@info "Plotting block allocations"
p2 = plot_block_allocations(shelves, blocks, S_m, H_s, W_s, b_bs, x_bs, z_bs)
